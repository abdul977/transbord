...
public class TemplateManager {
        ...
        try {
            ...
            for (Map.Entry<Integer, String> entry : templatePatterns.entrySet()) {
                try {
                    ...
                    if (matcher.find()) {
                        Log.d(TAG, "Detected built-in template type: " + entry.getKey());
                        return entry.getKey();
                    }
                } catch (Exception e) {
                    Log.e(TAG, "Error with pattern: " + entry.getValue() + ", error: " + e.getMessage());
            ...
            for (int i = 0; i < customTemplates.size(); i++) {
                try {
                    ...
                    Matcher matcher = pattern.matcher(text);
                    if (matcher.find()) {
                        Log.d(TAG, "Detected custom template: " + template.getName());
                        return TEMPLATE_CUSTOM + i; // Custom template IDs start after the built-in ones
                    }
                } catch (Exception e) {
                    Log.e(TAG, "Error with custom pattern: " + customTemplates.get(i).getPattern() + ", error: " + e.getMessage());
                    // Continue with next pattern
                }
            }
        } catch (Exception e) {
            Log.e(TAG, "Error detecting template type: " + e.getMessage());
        }

        Log.d(TAG, "No template detected for text");
    ...
    public String formatText(String text, int templateType) {
        ...
        try {
            Log.d(TAG, "Formatting text with template type: " + templateType);
            ...
            if (templateType >= TEMPLATE_CUSTOM) {
                ...
                if (customIndex < customTemplates.size()) {
                    ...
                    Log.d(TAG, "Applying custom template: " + template.getName());
                    return applyFormat(text, template.getFormat());
                } else {
                    Log.e(TAG, "Custom template index out of bounds: " + customIndex + ", size: " + customTemplates.size());
                }
            } else {
                // Built-in template
                String format = templateFormats.get(templateType);
                if (format != null) {
                    Log.d(TAG, "Applying built-in template: " + templateType);
                    return applyFormat(text, format);
                } else {
                    Log.e(TAG, "No format found for template type: " + templateType);
                }
            }
        } catch (Exception e) {
            Log.e(TAG, "Error formatting text: " + e.getMessage());
            Toast.makeText(context, "Error applying template: " + e.getMessage(), Toast.LENGTH_SHORT).show();
        }
